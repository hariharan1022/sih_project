...
export const FHIRBundleModal: React.FC<FHIRBundleModalProps> = ({ sessionId, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
      <div className="bg-slate-900 border border-cyan-500/40 rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">

        <div className="p-5 bg-slate-800 border-b border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Code className="w-6 h-6 text-cyan-400" />
            <div>
              <h3 className="text-xl font-bold text-white">HL7 FHIR R4 Document Bundle (ABDM-Ready)</h3>
              <p className="text-xs text-slate-400">Exportable FHIR Patient, Condition, and MedicationStatement resources</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {loading ? (
            <div className="py-12 text-center text-slate-400">Generating FHIR Bundle resources...</div>
          ) : (
            <pre className="p-4 bg-slate-950 text-xs text-cyan-300 font-mono rounded-xl border border-slate-800 overflow-x-auto max-h-[50vh]">
        ...
        <div className="p-5 bg-slate-800 border-t border-slate-700 flex items-center justify-between">
          ...
          <button
            ...
            disabled={!fhirData}
            className="px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-sm rounded-xl shadow-md flex items-center gap-2"
          >
